...
public class TC_176_AdministerNonPackedPRN_byAIN extends GetBrowserInstance
{
	WebDriver driver;
	...
	String vansahTestcaseName = "TC_176_AdministerNonPackedPRN_byAIN";
	String testStep = "";
	...
	@Test(priority = 1)
	public void administerNonPackedPRN_byAIN() 
	{
		...
		try
		{
			...
			if(homePage.verify_presenceOfOpenDoseRound(driver))
			{
				...
				Thread.sleep(6000);
				// Checked the POPup Type and Confirm Accordingly
				testStep = "User verifies the authentication popup";
				if(residentPage.presence0f_PinAuthenticationPopUpLocated(driver)){

					//Enter steps for Entering Pin
					testStep = "User enters RN pin and click 'verify' for authorization";
					residentPage.enterPin(driver, Constants.RN_PIN);
					residentPage.clickOn_verifyPin(driver);
					vansah.sendUpdateLog(vansahPackage, vansahRequirement, vansahTestCaseID, vansahRelease, vansahBuild,
							bhs_Environment,"pass", "Test Step: " + testStep +" - Passed!", BHSLog_bit);
	...
	@Test(dependsOnMethods = "administerNonPackedPRN_byAIN")
