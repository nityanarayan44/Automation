...
public class TC_198_AdministerPackedAIN_UnpackeckRN extends GetBrowserInstance
{
	...
	String vansahTestcaseName = "TC_198_AdministerPackedAIN_UnckeckRN";
	String testStep = "";
	...
	@Test(priority = 1)
	public void verifyPackedAdminAIN_UnckeckRN() 
	{
		...
		try
		{
			...
			if(homePage.verify_presenceOfOpenDoseRound(driver))
			{
						...
						&& residentPage.presence0f_SelectNumberGivenReasonLocated(driver))
				{
					...
					String RN_Checkbox = "(//div[@id='packedRegularGrid']/div[2]//input[contains(@ng-model,'RnOverride')])[1]";
					testStep = "User clicks on RN checkbox";
					key.click(driver, "xpath", RN_Checkbox);
					vansah.sendUpdateLog(vansahPackage, vansahRequirement, vansahTestCaseID, vansahRelease, vansahBuild,
							bhs_Environment,"pass", "Test Step: " + testStep +" - Passed!", BHSLog_bit);
					
					testStep = "User enters pin and verify the authentication popup";
					if(key.checkElementDisplay(driver, "xpath", "//div[@id='doseMedSecondAuthenticationModal']/div/label"))
					{
					...
					Thread.sleep(2000);
					testStep = "User Verifies if the 'No. of Given' textbox is disabled";
					if(driver.findElement(By.xpath("//label[text()='Number Given']/following-sibling::input[@disabled='disabled']")).isDisplayed())
					{
						vansah.sendUpdateLog(vansahPackage, vansahRequirement, vansahTestCaseID, vansahRelease, vansahBuild,
								bhs_Environment,"pass", "Test Step: " + testStep +" - Passed!", BHSLog_bit);
					}else{
					...
					testStep = "User unchecks the RN checkedbox";
					key.click(driver, "xpath", RN_Checkbox);
					vansah.sendUpdateLog(vansahPackage, vansahRequirement, vansahTestCaseID, vansahRelease, vansahBuild,
							bhs_Environment,"pass", "Test Step: " + testStep +" - Passed!", BHSLog_bit);
					
					testStep = "User Verify if the 'No. of Given' textbox is Enabled";
					if(key.checkElementDisplay(driver, "xpath", "//label[text()='Number Given']/following-sibling::input")){
						vansah.sendUpdateLog(vansahPackage, vansahRequirement, vansahTestCaseID, vansahRelease, vansahBuild,
								bhs_Environment,"pass", "Test Step: " + testStep +" - Passed!", BHSLog_bit);
					...
					testStep = "User enters No. of Given value for packed Medicine";
					residentPage.enter_NumberGiven(driver);
					vansah.sendUpdateLog(vansahPackage, vansahRequirement, vansahTestCaseID, vansahRelease, vansahBuild,
							bhs_Environment,"pass", "Test Step: " + testStep +" - Passed!", BHSLog_bit);
	...
	@Test(dependsOnMethods = "verifyPackedAdminAIN_UnckeckRN")
